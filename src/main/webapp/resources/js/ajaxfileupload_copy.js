jQuery.extend({
    createUploadIframeMulti: function (id, uri) {//id为当前系统时间字符串，uri是外部传入的json对象的一个参数
        //create frame
    ...
    createUploadFormMulti: function (id, fileElementId, data) {//id为当前系统时间字符串，fileElementId为页面<input type='file' />的id，data的值需要根据传入json的键来决定
        //create form
    ...
    ajaxFileUploadMulti: function (s) {//这里s是个json对象，传入一些ajax的参数
        ...
        var form = jQuery.createUploadFormMulti(id, s.fileElementId, (typeof (s.data) == 'undefined' ? false : s.data)); //创建动态form
        var io = jQuery.createUploadIframeMulti(id, s.secureuri); //创建动态iframe
        ...
        var uploadCallback = function (isTimeout) {//回调函数
            ...
            try {                if (io.contentWindow) {//动态iframe所在窗口对象是否存在
                ...
                setTimeout(function () {//设置超时时间jQueryUtil/
                    try {
    ...
    uploadHttpDataMulti: function (r, type) {        var data = !type;
