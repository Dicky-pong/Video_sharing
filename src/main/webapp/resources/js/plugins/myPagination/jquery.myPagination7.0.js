		...
		function init(param,obj){
			...
			var defaults = {
			  ...
			  detailPanel:{
				  ...
				  selectPageSizeArr:[10,15,20,25,30]
			  }
            };
			...
			function getDetailPanelInfo() {
				...
				*/
				str += '<div class="mleft" style="left:'+getLeft()+';">';
				...
				if(opts.ajax.on){
					...
					$.ajax({
					  ...
					  success: function(data) {
						...
						responseHandle(data);
						createPageBar();
						opts.ajax.initComplete(data);//调用完成事件.
					  }
					});
				}else{
			...
			function responseHandle(data){
				...
				//如果当前页数，超出了返回的总页数，则将当前页数设置为总页数
				/*
				if(opts.currPage > resultPageCount){
					opts.currPage = resultPageCount;
				}
				*/
            ...
            function createPageBar() {
              ...
              obj.children("a").each(function(index,element) {
                ...
                el.click(function() {
                  ...
                  var page = parseInt(el.attr("paged"));
				  page = page > 0 ? page : 1;
                  //单击某一个 a  标签时，去除其他 a 标签的 click 事件
                  obj.children("a").each(function(j,jEl) {
                     $(jEl).unbind("click");
					 //在ie中防止跳到页面顶部.
					 $(jEl).click(function(){
						return false;
